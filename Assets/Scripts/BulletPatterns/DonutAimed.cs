...
public class DonutAimed : AimedBulletPattern
{
    ...
    protected override IEnumerator Fire()
    {
        ...
        while (true)
        {
            ...
            m_dx = bulletSpawn.position.x - m_targetPos.x;
            m_dy = bulletSpawn.position.y - m_targetPos.y;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                bulletInst.transform.position = bulletSpawn.position + new Vector3(radius * Mathf.Cos(twoPiPerCount * i), radius * Mathf.Sin(twoPiPerCount * i), 0f);
                bulletInst.transform.rotation = Quaternion.Euler(Vector3.forward * (theta * i - 90f));
                bulletInst.SetActive(true);

                if (m_dx != 0f)
                {
                    if (m_targetPos.x <= bulletSpawn.position.x)
